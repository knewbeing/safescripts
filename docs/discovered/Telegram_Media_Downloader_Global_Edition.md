...
> 该脚本未检测到数据外传、隐私采集、远程代码执行、代码混淆或 DOM XSS 风险。主要问题为申请了未使用的高权限（unsafeWindow）和未锁定更新源，属于中低风险。整体安全性较高，建议移除未使用权限并关注供应链完整性。
...
**🟠 MEDIUM** — 权限滥用  
> 申请了 @grant unsafeWindow 权限，但代码中未使用该权限。  
> 位置：元数据 @grant  
> 建议：移除未使用的高权限申请，减少潜在攻击面。

**🟠 MEDIUM** — 供应链风险  
> 未锁定 @downloadURL 和 @updateURL 版本哈希，存在供应链污染风险。  
> 位置：元数据 @downloadURL/@updateURL  
> 建议：建议固定版本哈希或定期核查更新源完整性。
