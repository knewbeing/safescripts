...
(function () {
    ...
    /**
     * Torn's item market uses CSS Modules: generated classes are `localName___<buildHash>`.
     * Never match the full class including the hash — it changes every deploy.
     */
    ...
    /** Descendant of an ancestor whose class contains `ancestorContains` (e.g. `itemsHeader___`). */
    function tornScopedDescendant(ancestorContains, localName) {
        const p = `${localName}___`,
              a = `[class*="${ancestorContains}"]`;
    ...
    function getCache(itemId) {
        try {
            ...
            /* corrupt or legacy cache payload */
        }
        return null;
    }
    ...
    function createListingCard(listing, index) {
        ...
        try {
            ...
            /* invalid or missing visited-state JSON */
        }
    ...
    function processSellerWrapper(wrapper) {
        ...
        const nameEl = itemTile.querySelector(tornCssModuleSelector('name')),
              btn = nameEl?.closest(tornCssModuleSelector('item'))?.querySelector('button[aria-controls^="wai-itemInfo-"]')
                  ?? itemTile.querySelector('button[aria-controls^="wai-itemInfo-"]');
        if (nameEl && btn) {
    ...
    function processMobileSellerList() {
        ...
        const btn = headerEl?.closest(itemsHeaderSel)?.querySelector('button[aria-controls^="wai-itemInfo-"]')
            ?? document.querySelector(`${itemsHeaderSel} button[aria-controls^="wai-itemInfo-"]`);
        let itemId = "unknown";
        if (btn) {
    ...
    function processAllSellerWrappers(root = document.body) {
        if (checkMobileView()) return;
        const sellerWrappers = root.querySelectorAll(tornCssModuleSelector('sellerListWrapper'));
    ...
    const observer = new MutationObserver(mutations => {
        ...
        mutations.forEach(mutation => {
            ...
            mutation.removedNodes.forEach(node => {
                if (node.nodeType === Node.ELEMENT_NODE &&
                    node.matches(tornUlCssModuleSelector('sellerList')) &&
                    checkMobileView()) {
    ...
    if (window.location.href.includes("bazaar.php")) {
        ...
        if (targetItemId && highlight === "1") {
            ...
            function highlightItem() {
                ...
                const card = img ? img.closest(tornCssModuleSelector('item')) : null;
