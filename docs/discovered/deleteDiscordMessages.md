...
该脚本可以帮助用户批量删除Discord频道或私信中的所有消息。安装后，用户可以快速清理聊天记录，节省手动删除的时间。适用于需要管理大量消息的Discord用户。
...
2. 进入需要清理消息的频道或私信窗口。
3. 使用脚本提供的界面或按钮执行批量删除操作。
4. 等待脚本自动删除频道或私信中的所有消息。
...
**🔴 HIGH** — 权限滥用  
> 脚本未申请任何权限（@grant none），且代码中未使用任何高权限API，无权限滥用。  
> 位置：元数据与代码  
> 建议：无需修改，权限申请合理。
...
**🟡 LOW** — 外部依赖  
> 脚本未通过@require加载任何外部依赖，所有代码均为本地定义。  
> 位置：元数据  
> 建议：无需修改，避免引入不可信依赖。
