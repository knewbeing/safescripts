...
(function () {
  ...
  let rememberedData = JSON.parse(
    localStorage.getItem("hf-crime-profitability-data"),
  );
  let rememberedCrackingData = JSON.parse(
    localStorage.getItem("hf-crime-profitability-cracking-data"),
  );
  let lastFetched = Number(
    localStorage.getItem("hf-crime-profitability-last-fetched"),
  );
  let rememberedBFS = Number(
    localStorage.getItem("hf-crime-profitability-bfs"),
  );
  ...
  let savedSettings = JSON.parse(
    localStorage.getItem("hf-crime-profitability-settings"),
  );
  if (savedSettings) settings = savedSettings;

  let threshold = 0;
  let savedThreshold = Number(
    localStorage.getItem("hf-crime-profitability-threshold"),
  );
  ...
  function crimePage(data, crackingData, observed, retries = 30) {
    let list = document.body.querySelector(`[class*="virtualList__"]`);
    let subCrimes = list?.querySelectorAll(`[class*="crimeOptionSection__"]`);

    // If DOM isn't fully loaded, try again
    if (!list || !subCrimes || subCrimes.length < 2) {
      if (retries > 0) {
        setTimeout(
          () => crimePage(data, crackingData, observed, retries - 1),
          100,
        );
      } else {
        console.warn(
          "[HF] Gave up looking for Crime Page subtitles after 30 retries.",
        );
      }
      return;
    }
    ...
    if (window.location.href.includes("pickpocketing") && !observed)
      createObserver(list, data);
    ...
    if (window.location.href.includes("cracking")) {
      ...
      let rig = document.body.querySelector(
        `[class*="strength__"] [class*="value__"]`,
      );
    ...
    for (let crimeData of data) {
      if (burglaryPage) {
        ...
        if (crimeData.target.includes("Focused")) {
          dataMap.set(
            crimeData.target.replace(" (Focused)", "").toLowerCase(),
            crimeData,
          );
        } else if (crimeData.target.includes("Average")) {
          dataMap.set(
            `any ${crimeData.target.replace(" (Average)", "")}`.toLowerCase(),
            crimeData,
          );
        }
      } else {
        if (crimeData.target === "City Centre")
          crimeData.target = "City Center"; // Issue in the sheet for graffiti
        if (crimeData.target)
          dataMap.set(crimeData.target.toLowerCase(), crimeData);

        // If forgery, find the best forgery target!
        if (
          forgeryPage &&
          crimeData.crime === "Forgery" &&
          parseInt(crimeData["$/N"]?.replace(/[$,]/g, "")) > maximumForgery
        ) {
    ...
    if (window.location.href.includes("cracking") && crackingData) {
      for (let crimeData of crackingData) {
        if (crimeData["Targeted service "])
          crackingDataMap.set(
            crimeData["Targeted service "].toLowerCase(),
            crimeData,
          );
      }
    }
    ...
    for (let subCrime of subCrimes) {
      let firstTextNode = Array.from(subCrime.childNodes).find(
      ...
      // PICKPOCKETING
      let pickpocketing = subCrime.querySelector(
        `[class*="titleAndProps__] div'`,
      );
      if (pickpocketing) label = pickpocketing.textContent.trim().toLowerCase();

      // CRACKING
      let cracking = subCrime.querySelector(`[class*="type__"]`);
      let crackingService = subCrime.querySelector(`[class*="service__"]`);

      // FORGERY (MOBILE)
      let mobileForgery = subCrime.querySelector(
        `[class*="tabletProjectTitle__"]`,
      );
      if (mobileForgery) {
        label = mobileForgery.textContent.trim().toLowerCase();
        firstTextNode = Array.from(mobileForgery.childNodes).find(
          (node) => node.nodeType === Node.TEXT_NODE,
        );
      }
      ...
      // FORGERY
      if (forgeryPage) {
        let dropdownWrapper = subCrime.querySelector(
          `[class*="optionWithLevelRequirement__"]`,
        );
        if (
          (!mobileForgery && subCrime.textContent === "Begin a New Project") ||
          dropdownWrapper
        ) {
      ...
      if (burglaryPage) {
        let dropdownWrapper = subCrime.querySelector(
          `[class*="dropdownWrapper__"]`,
        );
        if (dropdownWrapper) burglary(subCrime, dataMap);
      }

      // SEARCH FOR CASH (MOBILE)
      let mobileSearchForCash = subCrime.querySelector(
        `[class*="titleAndIcon__"]`,
      );
      if (mobileSearchForCash) {
        firstTextNode = Array.from(mobileSearchForCash.childNodes).find(
          (node) => node.nodeType === Node.TEXT_NODE,
        );
      ...
      // GRAFFITI (MOBILE)
      let mobileGraffiti = subCrime.querySelector(
        `[class*="tabletTitleAndTagCount__"]`,
      );
      if (mobileGraffiti) {
        firstTextNode = Array.from(mobileGraffiti.childNodes).find(
          (node) => node.nodeType === Node.TEXT_NODE,
        );
        label = firstTextNode?.textContent.trim().toLowerCase();
      }
      ...
      // SHOPLIFTING (MOBILE)
      let mobileShoplifting = subCrime.querySelector(
        `[class*="tabletShopTitle__"]`,
      );
      if (mobileShoplifting)
        label = mobileShoplifting.textContent.trim().toLowerCase();

      // MOBILE MISC
      let mobileOther = !document.body.querySelector(
        `[class*="searchFormWrapper__"]`,
      );

      // MOBILE BURLGARY
      if (mobileOther && burglaryPage) {
        let title = subCrime.querySelector(`[class*="title__"]`);
      ...
      // GRAFFITI (MOBILE)
      if (mobileGraffiti && !crimeData)
        crimeData = dataMap.get(`${label} district`);
      ...
      if (!mobileSearchForCash && !mobileShoplifting)
        subCrime.style.justifyContent = "space-between";

      // MOBILE STYLE CHANGES
      let typeServiceWrapper = subCrime.querySelector(
        `[class*="typeAndServiceWrapper__"]`,
      );
      ...
      if (crimeData["$/N"]) {
        // Information found in the sheet
        ...
        if (moneyPerNerve < threshold)
          span.style.color = "var(--default-base-gold-color)";
        if (moneyPerNerve < 0)
          span.style.color = "var(--default-base-important-color)";
      ...
      subCrime.parentNode.parentNode.parentNode.parentNode.setAttribute(
        "data-hf-value",
        moneyPerNerve,
      );
      ...
      if (!existingSpan) {
        if (cracking) {
          ...
          mobileSearchForCash.insertBefore(
            span,
            mobileSearchForCash.childNodes[1] || null,
          );
        } else if (mobileOther && burglaryPage) {
          let titleAndProgress = subCrime.querySelector(
            `[class*="titleAndProgress__"]`,
          );
          let progressBar = titleAndProgress.querySelector(
            `[class*="progressBar__"]`,
          );
      ...
      let graffiti = subCrime.querySelector(
        `[class*="reputationIconWrapper__]"`,
      );
      ...
      if (burglaryPage && !mobileOther) {
        ...
        let abandonButtonWrapper = subCrime.querySelector(
          `[class*="abandonButtonWrapper__"]`,
        );
    ...
    if (
      maximumElement &&
      maximum > threshold &&
      settings["Highlight most profitable crime"]
    )
  ...
  function findCrackingBFS(retries = 60) {
    ...
    if (retries > 0) {
      ...
      console.warn(
        "[HF] Gave up looking for Cracking Rig after 60 tries (1 per second).",
      );
    }
  }
  ...
  function bootlegging(data, retries = 30) {
    let optionWrappers = document.body.querySelectorAll(
      `[class*="crimeOptionWrapper__"]`,
    );
    ...
    if (!optionWrappers || optionWrappers.length < 2) {
      if (retries > 0) {
        ...
        console.warn(
          "[HF] Gave up looking for Bootlegging subtitles after 30 retries.",
        );
      }
      return;
    }
    ...
    for (let optionWrapper of optionWrappers) {
      ...
      if (
        !optionWrapper.textContent.toLowerCase().includes("sell") &&
        !optionWrapper.textContent.toLowerCase().includes("online")
      )
        continue;

      let options = optionWrapper.querySelectorAll(
        `[class*="crimeOptionSection__"]`,
      );
      for (let option of options) {
        ...
        if (match && match["$/N"])
          moneyPerNerve = parseInt(match["$/N"].replace("$", ""));
    ...
    if (maximum > threshold && settings["Highlight most profitable crime"])
      maximumElement.style.background = "var(--default-bg-green-hover-color)";
  }

  // Display value per nerve on DISPOSAL
  function disposal(data, observed, retries = 30) {
    let mobile = !document.body.querySelector(`[class*="searchFormWrapper__"]`);
    let list = document.body.querySelector(`[class*="virtualList__"]`);
    let subCrimes = list?.querySelectorAll(`[class*="crimeOptionSection__"]`);
    ...
    if (!list || !subCrimes || subCrimes.length < 2) {
      if (retries > 0) {
        ...
        console.warn(
          "[HF] Gave up looking for Disposal subtitles after 30 retries.",
        );
      }
      return;
    }
    ...
    for (let crimeData of data) {
      ...
      let [targetTitle, targetType] = crimeData.target
        .split(":")
        .map((s) => s.trim());
      ...
      disposalData[targetTitle.trim().toLowerCase()][targetType] = parseInt(
        crimeData["$/N"]?.replace(/[$,]/g, ""),
      );
    }
    ...
    for (let subCrime of subCrimes) {
      ...
      let methods = subCrime.parentNode.querySelectorAll(
        `[class*="methodButton__"]`,
      );
      for (let method of methods) {
        ...
        if (settings["Disposal Method Colours"]) {
          if (info < 0)
            method.style.border =
              "1px solid var(--default-base-important-color)";
          if (info >= 0)
            method.style.border = "1px solid var(--default-base-gold-color)";
          if (ariaLabel.trim().toLowerCase() === maximumType.toLowerCase())
            method.style.border = "1px solid var(--default-base-green-color)";
        }
      }
      ...
      if (maximumValue < threshold)
        div.style.color = "var(--default-base-gold-color)";
      if (maximumValue < 0)
        div.style.color = "var(--default-base-important-color)";
  ...
  function forgery(subCrime, dataMap, maximum, maximumTarget) {
    let mobileForgery = document.body.querySelector(
      `[class*="tabletProjectTitle__"]`,
    );
    let mobile = !document.body.querySelector(`[class*="searchFormWrapper__"]`);
    ...
    if (dropdownWrapper) {
      ...
      // Don't just keep adding info!
      let existingMainSpan = document.body.querySelector(
        ".hf-selected-dropdown-value",
      );
      if (existingMainSpan) return;

      let selectedCrime = dropdownWrapper.textContent
        .replace("Level", "")
        .trim();
      ...
      if (value < threshold)
        mainSpan.style.color = "var(--default-base-gold-color)";
      if (value < 0)
        mainSpan.style.color = "var(--default-base-important-color)";
  ...
  function burglary(subCrime, dataMap, listening) {
    ...
    if (listening)
      subCrime = document.body.querySelector(
        `[class*="propertyTypeSection__"]`,
      );

    let categoryButtons = subCrime.querySelectorAll(
      `[class*="targetCategoryButtons__"]`,
    );
    ...
    if (!mobile) {
      ...
      div.style.color = "var(--default-base-green-color)";
      div.style.flexDirection = "column";
      if (bestOptionValue < threshold)
        div.style.color = "var(--default-base-gold-color)";
      if (bestOptionValue < 0)
        div.style.color = "var(--default-base-important-color)";
      ...
      let categoryButtonsWrapper = subCrime.querySelector(
        `[class*="targetCategoryButtons__"]`,
      );
    ...
    let dropdownWrapper = document.body.querySelector(
      `[class*="dropdownMainWrapper__" button]`,
    );
    ...
    for (let list of lists) {
      ...
      let item = list.querySelector(`[class*="dropdownItem__"]`);
  ...
  function sortButtonClick(list, button, invalidOpposite) {
    ...
    subCrimeArray.sort((a, b) => {
      ...
      if (invalidOpposite && !a.className.includes("virtualItemsBackdrop__")) {
    ...
    let backDrop = list.querySelector(`[class*="virtualItemsBackdrop__"]`);
  ...
  function crimeHub(data, retries = 30) {
    ...
    if (!crimeHubRoot || !crimes || crimes.length < 2) {
      if (retries > 0) {
        ...
        console.warn(
          "[HF] Gave up looking for the crime hub after 30 retries.",
        );
      }
      return;
    }
    ...
    for (let crime of crimes) {
      let titleElement = crime.querySelector(`[class*="crimeTitle__"]`);
  ...
  function findMaximumProfit(data, crimeTitle, crimeElement) {
    ...
    let titleBar = crimeElement.querySelector(`[class*="titleBar__"]`);
    if (!titleBar) {
      titleBar = crimeElement.querySelector(`[class*="titleAndStatus__"]`);
      let statusGroups = titleBar.querySelector(`[class*="statusGroups__"]`);
  ...
  function fetchData() {
    ...
    if (
      rememberedCrackingData &&
      lastFetched &&
      currentEpoch - lastFetched <= twentyFourHours
    ) {
        ...
        onload: function (response) {
          ...
          if (type === "all") {
            ...
            localStorage.setItem(
              "hf-crime-profitability-data",
              JSON.stringify(data),
            );
          } else if (type === "cracking") {
            cachedCrackingData = data;
            localStorage.setItem(
              "hf-crime-profitability-cracking-data",
              JSON.stringify(data),
            );
          }
          ...
          if (completed === total) {
            let currentEpoch = Math.floor(Date.now() / 1000);
            localStorage.setItem(
              "hf-crime-profitability-last-fetched",
              currentEpoch,
            );
  ...
  function createSettingsButton() {
    ...
    if (!headerElement)
      headerElement = document.body.querySelector(".content-title");
  ...
  function createModal() {
    ...
    infoSpan.textContent =
      "Enable/disable which crimes you want crime profitability to appear for here.";
    ...
    cancelButton.addEventListener("click", function () {
      ...
      localStorage.setItem(
        "hf-crime-profitability-settings",
