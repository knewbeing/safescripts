...
> 该脚本未检测到数据外传、隐私采集、远程代码执行、代码混淆、DOM XSS、供应链风险、敏感 API 调用或 iframe 风险。唯一问题是申请了未使用的 unsafeWindow 高权限，建议移除。整体安全风险较低。未发现关键安全隐患。
...
**🟠 MEDIUM** — 权限滥用  
> 申请了 unsafeWindow 权限，但实际代码未使用 unsafeWindow。此权限可导致脚本与页面 JS 相互访问，增加安全风险。  
