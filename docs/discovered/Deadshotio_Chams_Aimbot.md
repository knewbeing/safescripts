...
> 该脚本未检测到数据外传、隐私采集、远程代码执行、代码混淆、DOM XSS、供应链风险、WebSocket 使用等高风险行为。唯一问题为申请了未使用的 unsafeWindow 高权限，建议移除。整体安全风险较低。
...
**🟠 MEDIUM** — 权限滥用  
> 脚本申请了 unsafeWindow 权限，但实际代码未使用 unsafeWindow，存在权限滥用风险。  
> 位置：元数据 @grant unsafeWindow  
> 建议：移除未使用的高权限申请，避免潜在安全隐患。
