...
def main() -> None:
    # ── 环境变量说明 ────────────────────────────────────────────────────
    # GITHUB_TOKEN        : GitHub Actions 自动注入，作用域仅限本仓库。
    #                       用于调用 GitHub Models API（AI 分析）和 Trending 抓取兜底。
    #
    # TARGET_REPO         : 由 setup job (pick_repo.py) 写入 GITHUB_OUTPUT，
    #                       再通过 needs.setup.outputs.target_repo 传入本 job 的 env。
    #                       格式为 "owner/repo"，是本次要操作的目标仓库。
    #
    # RESOLVED_ORG_TOKEN  : 由 workflow 通过 secrets[token_env_name] 动态查找。
    #                       token_env_name 由 pick_repo.py 推导（如 ANOTHER_ORG_GITHUB_TOKEN）。
    #                       若该 secret 未在本仓库注册，此变量为空字符串。
    #
    # TARGET_REPO_TOKEN   : 通用兜底 PAT，当 RESOLVED_ORG_TOKEN 为空时使用。
    #                       需在本仓库 Settings → Secrets 中手动添加。
    # ───────────────────────────────────────────────────────────────────
    ...
    # 优先使用组织专属 token，不存在时降级到通用 token
    target_token = (
        os.environ.get("RESOLVED_ORG_TOKEN", "").strip()   # 优先：<ORG>_GITHUB_TOKEN 对应的值
        or os.environ.get("TARGET_REPO_TOKEN", "").strip()  # 兜底：TARGET_REPO_TOKEN
    )
    if not target_token:
        raise SystemExit(
            f"❌  找不到 '{target_repo}' 的访问 token。\n"
            f"    请在本仓库 Settings → Secrets 中添加对应的 org token 后重试。"
        )
